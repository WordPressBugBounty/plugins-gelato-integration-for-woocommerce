...
class GelatoMain
{
    ...
    public function create()
    {
        ...
        add_action(GelatoConfig::RESET_ACTION, [$this, 'reset_connection']);
        register_uninstall_hook(GelatoConfig::PLUGIN_SLUG, [__CLASS__, 'reset_connection']);
        register_deactivation_hook(GelatoConfig::PLUGIN_SLUG, [__CLASS__, 'reset_connection']);
        add_action('rest_api_init', function () {
            register_rest_route('wc/v3', GelatoConfig::REST_RESET_ROUTE, [
                'methods' => 'GET',
    ...
    {
        add_menu_page(
            __('Dashboard', GelatoConfig::TEXT_DOMAIN),
            GelatoConfig::MENU_TITLE,
            'manage_options',
            GelatoConfig::MENU_SLUG,
            [GelatoMain::class, 'tab_routing'],
    ...
    public function reset_connection_endpoint()
    {
    ...
    public function check_permission()
    {
        if (!wc_rest_check_manager_permissions('webhooks', 'delete')) {
            return new WP_Error(
                'gelato_cannot_reset',
                __('Sorry, you cannot reset this plugin.', GelatoConfig::TEXT_DOMAIN),
    ...
    public static function tab_routing()
    {
        $tab = !empty($_GET['tab'])
            ...
            : GelatoPage::PAGE_ID_MAIN;

        if (isset($_GET['reset_plugin'])) {
            do_action(GelatoConfig::RESET_ACTION);
    ...
    private static function check_main_tests(): bool
    {
        $connector = new GelatoConnector();
